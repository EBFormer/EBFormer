...
default_config = dict(
    ...
    # > ... Although we wouldn't really expect to recompile a dynamic shape fusion in a model,
...
def fresh_start(config):
    ...
    if config.wandb:
        ...
        if config.get('sweep', False):
            config.run_name = (
                f"{config.run_name}_num_layers_{config.num_layers}_l_max_{config.l_max}_num_features_{config.num_features}"
                f"_num_basis_{config.num_basis}_conv_weight_shared_{config.conv_weight_shared}_lr_{config.learning_rate}")
